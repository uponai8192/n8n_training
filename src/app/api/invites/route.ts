...
const ALLOWED_INVITE_ROLES = new Set(["PARTNER", "ADMIN"]);

async function sendInviteEmail(email: string, url: string, role: string) {
  ...
  const result = await resend.emails.send({
    ...
    to: email,
    subject: `You're invited as ${role.toLowerCase()} — N8N + UponAI`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; background: #0f172a; color: #e2e8f0; border-radius: 16px;">
        ...
        <p style="color: #cbd5e1; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">
          You’ve been invited to join the platform as an <strong style="color: #fff;">${role.toLowerCase()}</strong>. Use the button below to create your account. This invite expires in <strong style="color: #fff;">7 days</strong>.
        </p>
...
export async function POST(req: Request) {
  ...
  const normalizedRole =
    typeof role === "string" && role.trim().length > 0 ? role.trim().toUpperCase() : "PARTNER";
  if (!ALLOWED_INVITE_ROLES.has(normalizedRole)) {
